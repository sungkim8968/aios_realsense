...
# AIOS Get root attributes Parameters: including device IP Get all basic attributes of AIOS, including serial number,
# bus voltage, motor temperature, inverter temperature, version number
...
def setRootConfig(dict, server_ip):
    data = {'method': 'SET', 'reqTarget': '/config',
            'dc_bus_overvoltage_trip_level': dict['dc_bus_overvoltage_trip_level'],
            'dc_bus_undervoltage_trip_level': dict['dc_bus_undervoltage_trip_level']}
    json_str = json.dumps(data)
    print("Send JSON Obj:", json_str)
    s.sendto(str.encode(json_str), (server_ip, PORT_srv))
    try:
...
def setPosition(position, velocity_ff, current_ff, reply_enable, server_ip, motor_number):
    data = {'method': 'SET', 'reqTarget': '/m0/setPosition', 'reply_enable': reply_enable, 'position': 0,
    ...
    print("Send JSON Obj:", json_str)
    s.sendto(str.encode(json_str), (server_ip, PORT_rt))
    # if reply_enable: try: data, address = s.recvfrom(1024) # print('Server received from {}:{}'.format(address,
    # data.decode('utf-8'))) json_obj = json.loads(data.decode('utf-8')) print("Position = %.2f, Velocity = %.0f,
    # Current = %.4f \n" %(json_obj.get('position'), json_obj.get('velocity'), json_obj.get('current'))) except
    # socket.timeout: # fail after 1 second of no activity print("Didn't receive anymore data! [Timeout]")
...
def setVelocity(velocity, current_ff, reply_enable, server_ip, motor_number):
    data = {'method': 'SET', 'reqTarget': '/m0/setVelocity', 'velocity': 0, 'current_ff': 0,
...
def setCurrent(current, reply_enable, server_ip, motor_number):
    data = {'method': 'SET', 'reqTarget': '/m0/setCurrent', 'current': 0, 'reply_enable': reply_enable}
    if motor_number == 0:
...
def setIOState(dict, reply_enable, server_ip):
    data = {'method': 'SET', 'reqTarget': '/IO_State', 'reply_enable': reply_enable, 'PWM0_CH': dict['PWM0_CH'],
            'PWM1_CH': dict['PWM1_CH'], 'SERVO0': dict['SERVO0'], 'SERVO1': dict['SERVO1']}
    json_str = json.dumps(data)
    print("Send JSON Obj:", json_str)
    s.sendto(str.encode(json_str), (server_ip, PORT_rt))
    if reply_enable:
        try:
            ...
            print('Server received from {}:{}'.format(
                address, data.decode('utf-8')))
            # json_obj = json.loads(data.decode('utf-8')) print("Position = %.2f, Velocity = %.0f, Current = %.4f \n"
            # %(json_obj.get('position'), json_obj.get('velocity'), json_obj.get('current')))
        except socket.timeout:  # fail after 1 second of no activity
            print("Didn't receive anymore data! [Timeout]")
...
def setNetworkSetting(dict, server_ip):
    data = {'method': 'SET', 'reqTarget': '/network_setting', 'DHCP_enable': dict['DHCP_enable'], 'SSID': dict['SSID'],
            'password': dict['password'], 'name': dict['name']}
    if not dict['DHCP_enable']:
